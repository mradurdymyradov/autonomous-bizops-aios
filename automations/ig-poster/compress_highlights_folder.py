...
def compress_all():
    ...
    print(f"Compressing {len(files)} highlight images into '{DEST_DIR}'...")
    ...
    print(f"\nFinished compressing {len(files)} files.")
    print(f"Total size: {total_orig/(1024*1024):.2f} MB -> {total_new/(1024*1024):.2f} MB (-{(1 - total_new/total_orig)*100:.1f}%)")
